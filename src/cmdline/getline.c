/* Based on: "$Id: getline.c,v 1.18 2004/07/17 22:47:36 rad2k Exp $"; */
...
/* small compilation fixes by <raddy at gmx dot net> */
#if defined(YSM_WITH_THREADS) && !defined(USE_READLINE)
#undef    read_size_t
#undef    write_size_t
#define    read_size_t    u_int32_t
#define    write_size_t    u_int32_t
...
#ifdef WIN32
static int      gl_termw = 95;        /* actual terminal width */
#else
...
static int      gl_cnt = 0, gl_pos = 0;    /* position and size of input */
...
for(;;){
  ...
  if (!key.ext_key) {
    ...
    switch (key.scancode) {
      ...
      break;
      case 82: /* insert */
      if (key.shift_key){ /* paste from clipbpard */
        int i;
        char *str=os2_get_clip();
        if(str){
          for (i=0;i<(strlen(str));i++) gl_addchar(str[i]);
          free(str);
        }
      }
      else {
        os2_set_edit_cursor(!gl_overwrite); /* changing cursor type */
        c=15;
      }
      break;
      case 83: /* delete key */
...
#ifdef WIN32
int8_t fl = 0;
    fl = g_promptstatus.flags;
    g_promptstatus.flags &= ~FL_BUSYDISPLAY;
    PRINTF(VERBOSE_BASE, (int8_t *)buf);
    g_promptstatus.flags = fl;
...
getline(char *prompt)
{
    ...
    while ((c = gl_getc()) != (GL_FAIL)) {
    if (gl_buf[0] == 0x00 && YSM_ConsoleIsHotKey(c))
        continue; /* rad2k modification.. */
